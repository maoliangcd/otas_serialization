...
inline constexpr void update_check_code(unsigned int &code, unsigned int sub_code)
{
    code *= 998244353;
    code += sub_code;
    return ;
...
struct check_code_struct {
    constexpr static unsigned int check_code_template() {
        ...
        if constexpr (map_container<T>) {
            update_check_code(code, check_code_helper<typename T::key_type>::value);
            update_check_code(code, check_code_helper<typename T::mapped_type>::value);
        } else if constexpr (normal_container<T>) {
            update_check_code(code, check_code_helper<typename T::value_type>::value);
        } else if constexpr (!std::is_arithmetic_v<T>) {
            constexpr auto count = get_member_count<T>();
            auto members = member_tuple_helper<T, count>::static_tuple_view();
            [&]<std::size_t... index>(std::index_sequence<index...>) {
                ((update_check_code(code, check_code_helper<std::remove_pointer_t<remove_cvref_t<std::tuple_element_t<index, decltype(members)>>>>::value)), ...);
...
struct check_code_struct<std::pair<T, U>> {
    constexpr static unsigned int check_code_template() {
        ...
        update_check_code(code, check_code_helper<T>::value);
        update_check_code(code, check_code_helper<U>::value);
        return code;
    }
};
...
struct check_code_struct<std::tuple<Args...>> {
    constexpr static unsigned int check_code_template() {
        ...
        [&]<std::size_t... index>(std::index_sequence<index...>) {
            ((update_check_code(code, check_code_helper<remove_cvref_t<Args>>::value)), ...);
